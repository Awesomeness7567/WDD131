...
// Generates the Star Rating HTML markup depending on the decimal rating
function generateRatingHTML(rating) {
    ...
    for (let i = 1; i <= 5; i++) {
        if (i <= fullStars) {
            ...
            // Using a simple unicode star split representation or just standard star for basic display
            starsHTML += '<span aria-hidden="true" class="icon-star">⭐</span>'; 
        } else {
...
// Converts a single recipe object into HTML Template structure matching your design
function createRecipeCardTemplate(recipe) {
    // Generate markup for tags
    const tagsHTML = recipe.tags.map(tag => `<span class="tag">${tag.toLowerCase()}</span>`).join(' ');
...
// Clears existing recipe cards or feedback elements and appends new HTML content
function renderRecipes(recipesToDisplay) {
    // Remove all currently drawn <main> elements or "no results" notices inside container
    const oldCards = container.querySelectorAll('.recipe-card, .no-results');
    ...
    if (recipesToDisplay.length === 0) {
        ...
        noResultsMessage.textContent = 'No recipes found matching your keyword. Try another search!';
        container.insertBefore(noResultsMessage, container.querySelector('.footer'));
        return;
    }

    // Build elements and inject right before the footer item
    recipesToDisplay.forEach(recipe => {
        const parser = new DOMParser();
        const doc = parser.parseFromString(createRecipeCardTemplate(recipe), 'text/html');
        const cardElement = doc.querySelector('.recipe-card');
        container.insertBefore(cardElement, container.querySelector('.footer'));
    });
}

// Handles the filter and sorting mechanism
function executionSearch() {
    const query = searchInput.value.trim().toLowerCase();
    
    if (!query) {
        // Optional: If search bar is cleared, display a random one again
        showRandomRecipe();
        return;
    }

    // Filter recipes matching tags, name, or description
    const filteredRecipes = recipes.filter(recipe => {
        ...
        const matchTags = recipe.tags.some(tag => tag.toLowerCase().includes(query));
        
        return matchName || matchDescription || matchTags;
    });

    // Alphabetical sort by recipe Name
    filteredRecipes.sort((a, b) => a.name.localeCompare(b.name));

    renderRecipes(filteredRecipes);
}

// Selects one random recipe and sends it to render
...
document.addEventListener('DOMContentLoaded', () => {
    // 1. Initial State: Display a random recipe
    showRandomRecipe();

    // 2. Action States: Search Triggered via button click
    searchButton.addEventListener('click', executionSearch);

    // 3. Action States: Search Triggered via typing "Enter" key
